...
- **[Auto Low Power Mode](#auto-low-power-mode)**: A shortcut that works with Autocuts to activate Low Power Mode automatically when your battery falls to a certain percentage.
...
The [Auto Low Power Mode shortcut](https://routinehub.co/shortcut/3641) is used with Autocuts to automatically turn on Low Power Mode when your battery reaches a certain percentage. By default, it performs the check once every five minutes thanks to its integration with LimitKit.
...
### Example 3: Auto DND
The [Auto DND](https://adamtow.github.io/auto-dnd) shortcut works with Autocuts to automatically toggle Do Not Disturb mode when the current time coincides with calendar events whose title or notes field has the string "DND".
...
- `command`: `value` | `test` | `track` | `delete`
- `interval`: used with the `test` call
- `unit`: used with the `test` call
- `properties`: used with `test` (update) and `track` calls
...
## Localization
LimitKit currently supports the English language, but the shortcut is fully ready to be localized. Visit the [LimitKit page on GitHub](https://github.com/adamtow/limitkit) and submit a pull request for a native localization file.